'use client';

import { useEffect, useMemo, useState, useTransition } from "react";

import Card from "@/components/Card";
import Button from "@/components/Button";
import { api } from "@/lib/axios";

const blankAchievement = {
  title: "",
  description: "",
  issuedBy: "",
  date: "",
  link: "",
  category: "general",
};

export default function AchievementsManager() {
  const [achievements, setAchievements] = useState([]);
  const [formState, setFormState] = useState(blankAchievement);
  const [editingId, setEditingId] = useState(null);
  const [status, setStatus] = useState(null);
  const [isPending, startTransition] = useTransition();

  const loadAchievements = async () => {
    try {
      const data = await api.getAchievements();
      setAchievements(data);
    } catch (error) {
      console.error("Failed to load achievements", error);
    }
  };

  useEffect(() => {
    loadAchievements();
  }, []);

  const sortedAchievements = useMemo(
    () => [...achievements].sort((a, b) => a.order - b.order || new Date(b.date || 0) - new Date(a.date || 0)),
    [achievements]
  );

  const resetForm = () => {
    setFormState(blankAchievement);
    setEditingId(null);
  };

  const handleChange = (event) => {
    const { name, value } = event.target;
    setFormState((prev) => ({ ...prev, [name]: value }));
  };

  const handleSubmit = (event) => {
    event.preventDefault();
    if (!formState.title) return;

    startTransition(async () => {
      try {
        const payload = {
          ...formState,
          date: formState.date ? new Date(formState.date).toISOString() : null,
        };
        if (editingId) {
          await api.updateAchievement(editingId, payload);
          setStatus({ type: "success", message: "Achievement updated" });
        } else {
          await api.createAchievement(payload);
          setStatus({ type: "success", message: "Achievement created" });
        }
        resetForm();
        await loadAchievements();
      } catch (error) {
        setStatus({
          type: "error",
          message: error?.response?.data?.message ?? "Unable to save achievement",
        });
      }
    });
  };

  const handleEdit = (achievement) => {
    setEditingId(achievement._id);
    setFormState({
      title: achievement.title ?? "",
      description: achievement.description ?? "",
      issuedBy: achievement.issuedBy ?? "",
      date: achievement.date ? achievement.date.slice(0, 10) : "",
      link: achievement.link ?? "",
      category: achievement.category ?? "general",
    });
  };

  const handleDelete = (id) => {
    startTransition(async () => {
      try {
        await api.deleteAchievement(id);
        setAchievements((prev) => prev.filter((achievement) => achievement._id !== id));
      } catch (error) {
        setStatus({
          type: "error",
          message: error?.response?.data?.message ?? "Unable to delete achievement",
        });
      }
    });
  };

  return (
    <Card className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-white">Achievements</h2>
          <p className="text-sm text-slate-400">Highlight certifications, awards, and milestones.</p>
        </div>
        {status && (
          <span
            className={`text-sm ${
              status.type === "success" ? "text-emerald-300" : "text-rose-300"
            }`}
          >
            {status.message}
          </span>
        )}
      </header>

      <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
        <label className="flex flex-col text-sm text-slate-200">
          Title
          <input
            required
            name="title"
            value={formState.title}
            onChange={handleChange}
            className="mt-1 rounded-lg border border-slate-700 bg-slate-900 px-4 py-2 text-white"
          />
        </label>
        <label className="flex flex-col text-sm text-slate-200">
          Issued by
          <input
            name="issuedBy"
            value={formState.issuedBy}
            onChange={handleChange}
            className="mt-1 rounded-lg border border-slate-700 bg-slate-900 px-4 py-2 text-white"
          />
        </label>
        <label className="flex flex-col text-sm text-slate-200">
          Date
          <input
            type="date"
            name="date"
            value={formState.date}
            onChange={handleChange}
            className="mt-1 rounded-lg border border-slate-700 bg-slate-900 px-4 py-2 text-white"
          />
        </label>
        <label className="flex flex-col text-sm text-slate-200">
          Category
          <input
            name="category"
            value={formState.category}
            onChange={handleChange}
            className="mt-1 rounded-lg border border-slate-700 bg-slate-900 px-4 py-2 text-white"
          />
        </label>
        <label className="md:col-span-2 flex flex-col text-sm text-slate-200">
          Description
          <textarea
            name="description"
            rows={3}
            value={formState.description}
            onChange={handleChange}
            className="mt-1 rounded-lg border border-slate-700 bg-slate-900 px-4 py-2 text-white"
          />
        </label>
        <label className="md:col-span-2 flex flex-col text-sm text-slate-200">
          Link (optional)
          <input
            name="link"
            value={formState.link}
            onChange={handleChange}
            className="mt-1 rounded-lg border border-slate-700 bg-slate-900 px-4 py-2 text-white"
          />
        </label>
        <div className="md:col-span-2 flex items-center gap-3">
          <Button as="button" type="submit" disabled={isPending} className="px-6 py-3">
            {isPending ? "Saving..." : editingId ? "Update" : "Create"}
          </Button>
          {editingId && (
            <button
              type="button"
              onClick={resetForm}
              className="text-sm text-slate-400 hover:text-slate-200"
            >
              Cancel editing
            </button>
          )}
        </div>
      </form>

      <div className="space-y-3">
        {sortedAchievements.map((achievement) => (
          <div
            key={achievement._id}
            className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-800 bg-slate-900/40 px-4 py-3"
          >
            <div>
              <p className="text-sm font-semibold text-white">{achievement.title}</p>
              <p className="text-xs text-slate-400">
                {achievement.issuedBy || "Unknown issuer"}
                {achievement.date ? ` · ${new Date(achievement.date).toLocaleDateString()}` : ""}
              </p>
            </div>
            <div className="flex gap-2 text-sm">
              <button
                className="rounded-full border border-slate-700 px-3 py-1 text-slate-200 hover:border-blue-400 hover:text-blue-200"
                type="button"
                onClick={() => handleEdit(achievement)}
              >
                Edit
              </button>
              <button
                className="rounded-full border border-rose-600 px-3 py-1 text-rose-200 hover:bg-rose-600/20"
                type="button"
                onClick={() => handleDelete(achievement._id)}
              >
                Delete
              </button>
            </div>
          </div>
        ))}
        {!sortedAchievements.length && (
          <p className="text-sm text-slate-400">No achievements yet. Add your first milestone above.</p>
        )}
      </div>
    </Card>
  );
}
